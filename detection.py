...
def sample_selection(df):
    try:
        ...
        # 将样本按类别规整便于调用模型
        # 复制到新的Dataframe
        lane_departure_samples = df.iloc[lane_departure].copy()
        ...
        logging.debug(f"[sample_selection] succeededed in classifying")
    except Exception as e:
        logging.error(f"[sample_selection] an error occurred|exception:{str(e)}|traceback:{traceback.format_exc()}")
...
# 跟车距离检测
    try:
        t1 = time.time()
        logging.debug(
            f"[sample_selection] len(distance_samples):{len(distance_samples)}")
        Test_path = []
        Test_path_video = []
        if len(distance_samples) > 0:
            ...
            Second_det_distance_video = vehicle_collision(
                Test_path_video)  # CLIP判定
            # 视频检测
            ...
            for index, row in distance_samples.iterrows():
                ...
                t_plate_type = f"{plate}_{t_type}"
                alarm_time = pd.to_datetime(row['alarm_begin_time'])
                repet, repet_label = is_repet(
                    plate_type, Time, t_plate_type, alarm_time)
            ...
            logging.debug(
                f"[sample_selection] second_det_distance:{Second_det_distance}")
            logging.debug(
                f"[sample_selection] merge_dis_distance:{Merge_dis_distance}")
            t2 = time.time()
        logging.debug("[sample_selection] succeeded in distance detection")
    except Exception as e:
        logging.error(f"[sample_selection] an error occurred|exception:{str(e)}|traceback:{traceback.format_exc()}")
#
# #分心行为检测
    logging.debug(f"[sample_selection] len(distracted_samples):{len(distracted_samples)}")
...
# # #行人碰撞检测
    ...
    logging.debug(f"[sample_selection] len(pedestrian_samples):{len(pedestrian_samples)}")
    Test_path = []
    Test_path_video = []
    t1 = time.time()
    if len(pedestrian_samples) > 0:
        ...
        Second_det_pedestrian_video = pedestrian_collision(
            Test_path_video)  # CLIP判定
        ...
        logging.debug(f"[sample_selection] second_det_pedestrian:{Second_det_pedestrian}")
        logging.debug(f"[sample_selection] merge_dis_pedestrian:{Merge_dis_pedestrian}")
        t2 = time.time()
        # print(t2-t1)
    logging.debug("[sample_selection] succeeded in pedestrian detection")
# #驾驶行为检测
    # print(len(behavior_samples))
    logging.debug(f"[sample_selection] len(behavior_samples):{len(behavior_samples)}")
    t1 = time.time()
    Test_path = []
    Test_path_video = []
    if len(behavior_samples) > 0:
        ...
        for i in range(len(Second_det_behavior)):
            # 是视频和图像都没有则标注为3
            if (not os.path.exists(Test_path[i])) and (not os.path.exists(Test_path_video[i])):
        ...
        logging.debug(f"[sample_selection] second_det_behavior:{Second_det_behavior}")
        logging.debug(f"[sample_selection] merge_dis_behavior:{Merge_dis_behavior}")
        t2 = time.time()
    logging.debug("[sample_selection] succeeded in behavior detection")

# 道路偏离检测
    # print(len(lane_departure_samples))
    logging.debug(
        f"[sample_selection] len(lane_departure_samples):{len(lane_departure_samples)}")
    t1 = time.time()
    Test_path = []
    Test_path_2 = []
    Test_path_video = []
    ...
    if len(lane_departure_samples) > 0:
        ...
        Second_det_lane_departure_video = lanelines.video_Lanelines.main(
            Test_path_video)
        ...
        logging.debug(
            f"[sample_selection] second_det_lane_departure|picture_pos_path:{picture_pos_path}")
        result = [4] * length
        ...
        logging.debug(
            f"[sample_selection] second_det_lane_departure:{Second_det_lane_departure}")
        logging.debug(
    ...
    logging.debug(f"[sample_selection] second_det_other:{Second_det_Other}")
    logging.debug(f"[sample_selection] merge_dis_other:{Merge_dis_Other}")
    df_combined = pd.concat([lane_departure_samples, distance_samples, behavior_samples,
                            distracted_samples, pedestrian_samples, Other_samples], axis=0)
    return df_combined
