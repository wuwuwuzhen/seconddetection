...
# 跟车距离检测
    try:
        t1 = time.time()
        logging.info(
            f"PID {pid}|len(distance_samples): {len(distance_samples)}")
        Test_path = []
        Test_path_video = []
        if len(distance_samples) > 0:
            ...
            for index, row in distance_samples.iterrows():
                ...
                if (Second_det_distance[a] == 1) and repet:  # 二次检测通过且重复
                    ...
                    logging.info(
                        f"PID {pid}|index {index}|Merge_dis_distance: {Merge_dis_distance[a]}")
                    a += 1
                    continue
                a += 1
            a = 0
            ...
            logging.info(
                f"PID {pid}|index {index}|Second_det_distance: {Second_det_distance}")
            logging.info(
                f"PID {pid}|index {index}|Merge_dis_distance: {Merge_dis_distance}")
            t2 = time.time()
            # print(t2-t1)
...
# #分心行为检测
    ...
    if len(distracted_samples) > 0:
        ...
        logging.info(
            f"PID {pid}|Second_det_distracted_2: {Second_det_distracted_2}")
...
# # #行人碰撞检测
    ...
    logging.info(
        f"PID {pid}|len(pedestrian_samples): {len(pedestrian_samples)}")
    Test_path = []
    Test_path_video = []
    t1 = time.time()
    if len(pedestrian_samples) > 0:
        ...
        logging.info(
            f"PID {pid}|Second_det_pedestrian: {Second_det_pedestrian}")
...
# 道路偏离检测
    ...
    logging.info(
        f"PID {pid}|len(lane_departure_samples): {len(lane_departure_samples)}")
    t1 = time.time()
    Test_path = []
    Test_path_2 = []
    Test_path_video = []
    ...
    if len(lane_departure_samples) > 0:
        ...
        logging.info(
            f"PID {pid}|Lane Departure Detection|lanelines.Lanelines.main Succeed")
        ...
        logging.info(
            f"PID {pid}|Second_det_lane_departure|picture_pos_path:{picture_pos_path}")
        result = [4] * length
        ...
        logging.info(
            f"PID {pid}|Second_det_lane_departure: {Second_det_lane_departure}")
        logging.info(
            f"PID {pid}|Merge_dis_lane_departure: {Merge_dis_lane_departure}")
        t2 = time.time()
        # print(t2 - t1)
