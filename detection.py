...
def sample_selection(df):
    ...
    for i in range(len(exception_type)):
        if exception_type[i] in [3, 4, 5, 6]:
            lane_departure.append(i)
            continue
        if exception_type[i] in [14, 15, 18, 19]:
            behavior.append(i)
            continue
        if exception_type[i] in [1,2,  7, 8]:
            distance.append(i)
            continue
        if exception_type[i] in [16, 17]:
            distracted.append(i)
            continue
        if exception_type[i] in [9,10]:
            pedestrian.append(i)
        else:
            Other.append(i)
    ...
    Second_det_Other = [1] * len(Other_samples);
...
# 跟车距离检测
    ...
    if len(distance_samples) > 0:
        ...
        for i in range(len(Second_det_distance)):
            if Second_det_distance_video[i] == 1:
...
#     print(len(distracted_samples))
    ...
    if len(distracted_samples)>0:
        ...
        for i in range(len(Second_det_distracted_2)):
            Second_det_distracted[i] = Second_det_distracted_2[i]
            if Second_det_distracted_2_video[i]==1:
                Second_det_distracted[i]=1
        a = 0
        ...
        for index, row in distracted_samples.iterrows():
            ...
            a += 1
        distracted_samples['checkStatus']=Second_det_distracted
...
# #行人碰撞检测
    ...
    if len(pedestrian_samples) > 0:
        ...
        for i in range(len(Second_det_pedestrian)):
            if Second_det_pedestrian_video[i] == 1:
...
#道路偏离检测
    ...
    Test_path_video = [];Test_path_video_2=[]
    if len(lane_departure_samples)>0:
        ...
        if not os.path.getsize(video_pos_path):  ###没有视频
        ...
        elif not os.path.getsize(picture_pos_path):  ###没有图片
        ...
        for index, row in lane_departure_samples.iterrows():
            ...
            if ((Second_det_lane_departure[a] == 1 or Second_det_lane_departure_video[a] == 1) and t_type in [5, 6]) or ((
                    Second_det_lane_departure[a] == 2 or Second_det_lane_departure_video[a] == 2) and t_type in [3, 4]) or (Second_det_lane_departure[a] == 3 or Second_det_lane_departure_video[a] == 3):
                Second_det_lane_departure[a] = 1


            elif ((Second_det_lane_departure[a] == 4 or Second_det_lane_departure_video[a] == 4) and t_type in [3, 4, 5, 6]):
                Second_det_lane_departure[a] = 2  ###居中检测

            elif ((Second_det_lane_departure[a] == 1 or Second_det_lane_departure_video[a] == 1) and t_type in [3, 4]) or ((
                    Second_det_lane_departure[a] == 2 or Second_det_lane_departure_video[a] == 2) and t_type in [5, 6]):
                Second_det_lane_departure[a] = 0  ###偏离方向相反
        ...
        for i in range(len(Second_det_lane_departure)):
            ...
            if (not os.path.exists(Test_path_2[i])) and (not os.path.exists(Test_path_video_2[i])):  # 是视频和图像都没有则标注为3
