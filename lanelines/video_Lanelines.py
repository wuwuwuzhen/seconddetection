import torch
from lanelines.video_rundef import args_setting
...
import cv2
import time
from lanelines.video_rundef import rename,resize,unetlstmtxt,detection
import lanelines.video_rundef
import os
...
def output_result(args,model, test_loader, device):
    ...
    with torch.no_grad():
        for sample_batched in test_loader:
            ...
            data.save(lanelines.video_rundef.save_path + "%s_data.jpg" % k)#red line on the original image
            img.save(lanelines.video_rundef.save_path + "%s_pred.jpg" % k)#prediction result
...
def main(video_path):
    ...
    if args.model == 'SegNet-ConvLSTM' or 'UNet-ConvLSTM':
        test_loader = torch.utils.data.DataLoader(
            RoadSequenceDatasetList(file_path=lanelines.video_rundef.test_path, transforms=op_tranforms),
            batch_size=args.test_batch_size, shuffle=False, num_workers=1)
    else:
        test_loader = torch.utils.data.DataLoader(
            RoadSequenceDataset(file_path=lanelines.video_rundef.test_path, transforms=op_tranforms),
            batch_size=args.test_batch_size, shuffle=False, num_workers=1)

    # load model and weights
    model = generate_model(args)
    model.to(device)
    class_weight = torch.Tensor(lanelines.video_rundef.class_weight)
    criterion = torch.nn.CrossEntropyLoss(weight=class_weight).to(device)

    pretrained_dict = torch.load(lanelines.video_rundef.pretrained_path,map_location=torch.device('cpu'))
