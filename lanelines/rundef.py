    ...
    with torch.no_grad():
        for sample_batched in test_loader:
            ...
            data, target = sample_batched['data'].to(
                device), sample_batched['label'].type(torch.LongTensor).to(device)
            output, feature = model(data)
            feature_dic.append(feature)
            pred = output.max(1, keepdim=True)[1]
            img = torch.squeeze(pred).cpu().unsqueeze(
                2).expand(-1, -1, 3).numpy()*255
        ...
        if item[-8:-4] != 'pred':  # 初始的图片的格式为jpg格式的（或者源文件是png格式及其他格式，后面的转换格式就可以调整为自己需要的格式即可，我习惯转成.jpg）
            ...
            for i in range(int(total_num/2)):
                if item == (format(str(i + 1)) + '_pred.jpg'):
                    ...
                    shrink = cv.resize(
                        255-image1, size, interpolation=cv.INTER_AREA)
                    cv_imwrite(file_path, shrink)
                    break
                else:
                    i = i + 1
...
def LaneLines(path, file):
    ...
    while True:
        ...
        lines = cv.HoughLinesP(img_mask, 1, np.pi / 180,
                               10, minLineLength=100, maxLineGap=100)
        ...
        if right_x < gray.shape[1] and left_x > 0:
            if xm < 0.4 * width:
                cv.putText(frame, "right", (450, 200),
                           cv.FONT_HERSHEY_PLAIN, 4, (0, 0, 255), 5)
                flagr = 1
                flag = 1
            elif xm > 0.6 * width:
                cv.putText(frame, "left", (80, 200),
                           cv.FONT_HERSHEY_PLAIN, 4, (0, 0, 255), 5)
                flagl = 1
                flag = 2
            elif xm < 0.5*width+3 and xm > 0.5*width-3:
                cv.putText(frame, "middle", (80, 200),
                           cv.FONT_HERSHEY_PLAIN, 4, (0, 0, 255), 5)
                flagm = 1
                flag = 5
            else:
