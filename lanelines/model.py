import torch
import lanelines.config
import torch.nn as nn
from torchvision import models
import torch.nn.functional as F
from lanelines.utils import *
import operator
from lanelines.config import args_setting
...
def generate_model(args):
    ...
    if args.model == 'SegNet-ConvLSTM':
        ...
        model =UNet_ConvLSTM(lanelines.config.img_channel, lanelines.config.class_num).to(device)
    elif args.model == 'UNet':
        model = UNet(lanelines.config.img_channel, lanelines.config.class_num).to(device)
    return model
...
class SegNet_ConvLSTM(nn.Module):
    def __init__(self):
        ...
        self.upconv1_block = nn.Sequential(
                                           ...
                                           nn.BatchNorm2d(64, eps=1e-05, momentum=0.1, affine=True),
                                           self.relu,
                                           nn.Conv2d(64, lanelines.config.class_num, kernel_size=(3, 3), padding=(1,1)),
                                           )
